...
const department_normalizer_1 = require("./department-normalizer");
...
const testTable = [
    ...engineeringTestCases.map((x) => [x, department_normalizer_1.departmentIds.engineering]),
    ...nonEngineeringTestCases.map((x) => [x, undefined]),
];
test.each(testTable)("normalizes %p into %p", (...args) => {
    const [input, output] = args;
    const job = {
        department: input[0] || "",
        title: input[1] || "",
    };
    expect((0, department_normalizer_1.getNormalizedDepartment)(job)).toEqual(output);
});

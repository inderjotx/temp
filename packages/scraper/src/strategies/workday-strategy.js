...
exports.getJobsFromWorkday = void 0;
    ...
    while (responseRaw.status === 200) {
        ...
        jobs.forEach((jobInfo) => {
            data.push({
                ...
                location: (jobInfo.title.instances[0].text.toLowerCase().includes("remote")
                    ? "Remote, "
                    : "") +
                    jobInfo.subtitles[1].instances[0].text.replace(/( - Other)?, More\.\.\./, " and other options"),
...
exports.getJobsFromWorkday = getJobsFromWorkday;
function parseCreatedAt(jobDataPart) {
    ...
    if (!jobDataPart || jobDataPart.instances[0].text === "Posted Today") {
        ...
        daysFromToday = jobDataPart.instances[0].text.match(/Posted (\d+)\+? Days Ago/)[1];
    }
